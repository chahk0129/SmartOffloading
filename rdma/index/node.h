...
class node_t{
	...
	uint64_t leftmost_ptr;
	uint64_t sibling_ptr;
	uint32_t level;
	int last_index;
	Key_t lowest;
	Key_t highest;
	...
	node_t(uint64_t left, uint32_t level, int last_index=0): version(0), leftmost_ptr(left), sibling_ptr(0), level(level), last_index(last_index), lowest(MIN_KEY<Key_t>), highest(MAX_KEY<Key_t>) { }
...
template <typename Key_t>
static constexpr size_t inode_cardinality = (PAGE_SIZE - sizeof(node_t<Key_t>)) / sizeof(entry_t<Key_t, uint64_t>);
...
class inode_t: public node_t<Key_t>{
    public:
	inode_t(uint64_t left, const Key_t& key, uint64_t right, uint32_t level=0): node_t<Key_t>(left, level){
	    entry[0].key = key;
	    entry[0].value = right;
	}

	inode_t(uint8_t level=0): node_t<Key_t>(level){
	    entry[0].value = 0;
	}
	...
	bool insert(const Key_t& key, uint64_t& value){
	    ...
	    memmove(&entry[idx+1], &entry[idx], sizeof(entry_t<Key_t, uint64_t>) * (cnt - idx));
	    entry[idx].key = key;
	    entry[idx].value = value;
	    this->last_index++;
	...
	inode_t<Key_t, Value_t>* split(Key_t& split_key){
	    ...
	    int m = cnt / 2;
	    int num = cnt - m;
	    memcpy(sibling->entry, entry+m+1, sizeof(entry_t<Key_t, uint64_t>) * num);
    ...
    private:
	entry_t<Key_t, uint64_t> entry[inode_cardinality<Key_t>];
} __attribute__((packed));
