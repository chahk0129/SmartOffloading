...
bool server_t::init_resources(){
	...
	if(i == 0){ // rpc messages + idx root space for MR[0]
	    msg_size = (sizeof(request_t) + sizeof(response_t)) * CLIENT_THREAD_NUM;
	    idx_size = ROOT_BUFFER_SIZE; // reserve root address space for an index in each mr
	    alloc_offset = memory_pool[i] + msg_size + idx_size;
	}
	else{ // idx root space for MR[i]
	    idx_size = ROOT_BUFFER_SIZE;
	    alloc_offset = memory_pool[i] + idx_size;
	}
	allocator[i] = new allocator_t(alloc_offset, memory_size[i] - msg_size - idx_size);
    }

    ret = transport->setup_connection();
    if(!ret){
	debug::notify_error("Failed to connect to Client");
	return false;
    }
...
void server_t::handle_message(int tid){
    ...
    bind_core(CLIENT_THREAD_NUM-1 - tid);
