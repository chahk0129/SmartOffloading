...
#define MAX_STATS(tpe, name) total_##name = std::max(total_##name, _stats[tid]->name);
#define STR(x) #x
#define XSTR(x) STR(x)
...
class stats_thread_t {
    public:
	void init(int thd_id);
	void clear();

	char _pad2[CACHELINE_SIZE];
...
class stats_t {
	...
	void clear(int tid);
	void add_debug(int thd_id, uint64_t value, uint32_t select);
	void commit(int thd_id);
	void abort(int thd_id);
	void print();
