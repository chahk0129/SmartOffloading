...
struct server_worker_meta{
    int sock;
    int gid_idx;
    union ibv_gid gid;
    uint32_t lid;
    uint32_t rkey[MR_PARTITION_NUM];
    uint32_t qpn[WORKER_THREAD_NUM];
};

struct worker_client_meta{
    int sock;
    int gid_idx;
    union ibv_gid gid;
    uint32_t lid;
    #if defined BATCH
    uint32_t qpn[NETWORK_THREAD_NUM];
    #elif defined BATCH2
    uint32_t qpn[WORKER_THREAD_NUM];
    #else
...
struct ibv_context* open_device(bool is_dpu=false);
