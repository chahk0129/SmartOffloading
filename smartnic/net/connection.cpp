...
void server_listen(struct server_worker_meta* local, struct server_worker_meta* remote){
...
void worker_connect(const char* ip, struct server_worker_meta* local, struct server_worker_meta* remote){
...
void worker_listen(struct worker_client_meta* local, struct worker_client_meta* remote){
    struct sockaddr_in addr;
    int sock;
    int on = 1;
    ...
    socklen_t sin_size = sizeof(struct sockaddr_in);
    while(1 && (fd = accept(sock, (struct sockaddr*)&remote_addr, &sin_size)) != -1){
        if(!connect_qp2(fd, local, remote)){
            debug::notify_error("Server failed to connect QP with worker");
            continue;
        }
        break;
...
void client_connect(const char* ip, struct worker_client_meta* local, struct worker_client_meta* remote){
    int sock = socket_connect(ip); // first connect to master server
    if(sock < 0){
        debug::notify_error("Failed to create TCP socket to worker %s", ip);
        return;
    }

    if(!connect_qp2(sock, local, remote)){
        debug::notify_error("Failed to connect QP with worker %s", ip);
        return;
    }
    close(sock);
}

bool connect_qp(int sock, struct server_worker_meta* local, struct server_worker_meta* remote){
    if(socket_data_sync(sock, sizeof(struct server_worker_meta), (char*)local, (char*)remote) < 0){
...
bool connect_qp2(int sock, struct worker_client_meta* local, struct worker_client_meta* remote){
    if(socket_data_sync(sock, sizeof(struct worker_client_meta), (char*)local, (char*)remote) < 0){
        debug::notify_error("Failed to exchange node metadata");
        return false;
    }
    return true;
