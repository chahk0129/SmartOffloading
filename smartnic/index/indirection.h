...
//#define DEFAULT_TABLE_SIZE (10000000)  // table size = 32MB
#define DEFAULT_TABLE_SIZE ((uint64_t)1024 * 1024 * 4)  // table size = 32MB
...
class indirection_table_t{
	...
	void clear_addr(const uint32_t node_id){
	    tab[node_id].store(0);
	}

	uint64_t get_addr(const uint32_t node_id){
	    return tab[node_id].load();
	}

	bool set(const uint32_t node_id, uint64_t addr){
	    uint64_t empty = 0;
