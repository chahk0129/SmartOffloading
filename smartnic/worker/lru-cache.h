...
public:
  ...
  bool insert_with_evict(const TKey& key, const TValue& value, bool& evicted, Accessor& evictAccessor);
  bool insert_with_evict(const TKey& key, const TValue& value, bool& evicted, TKey& evict_key, TValue& evict_value);
...
private:
   ...
   */
  void evict();
  bool evict(HashMapAccessor& evictAccessor);
  bool evict(TKey& evict_key, TValue& evict_value);
...
template <class TKey, class TValue, class THash>
bool ThreadSafeLRUCache<TKey, TValue, THash>::
insert_with_evict(const TKey& key, const TValue& value, bool& evicted, Accessor& evictAccessor) {
  // Insert into the CHM
  ListNode* node = new ListNode(key);
  HashMapAccessor hashAccessor;
  HashMapValuePair hashMapValue(key, HashMapValue(value, node));
  ...
  if (size >= m_maxSize) {
    ...
    // Do not decrement m_size, since that would cause other threads to
    // inappropriately omit eviction during their own inserts.
    evicted = evict(evictAccessor.m_hashAccessor);
  }
...
template <class TKey, class TValue, class THash>
bool ThreadSafeLRUCache<TKey, TValue, THash>::
insert_with_evict(const TKey& key, const TValue& value, bool& evictionDone, TKey& evict_key, TValue& evict_value) {
  // Insert into the CHM
  ListNode* node = new ListNode(key);
  HashMapAccessor hashAccessor;
  HashMapValuePair hashMapValue(key, HashMapValue(value, node));
  ...
  if (size >= m_maxSize) {
    ...
    // Do not decrement m_size, since that would cause other threads to
    // inappropriately omit eviction during their own inserts.
    evictionDone = evict(evict_key, evict_value);
  }
...
template <class TKey, class TValue, class THash>
bool ThreadSafeLRUCache<TKey, TValue, THash>::
evict(HashMapAccessor& evictAccessor) {
  std::unique_lock<ListMutex> lock(m_listMutex);
  ListNode* moribund = m_tail.m_prev;
  if (moribund == &m_head) {
    // List is empty, can't evict
  ...
  if (!m_map.find(evictAccessor, moribund->m_key)) {
    // Presumably unreachable
    return false;
  }
...
evict(TKey& key, TValue& value) {
  ...
  //key = moribund->m_key;
  key = hashAccessor->first;
  value = hashAccessor->second.m_value;
