...
insert_with_evict(const TKey& key, const TValue& value, bool& evicted, Accessor& evictAccessor) {
  return getShard(key).insert_with_evict(key, value, evicted, evictAccessor);
}

template <class TKey, class TValue, class THash>
bool ThreadSafeScalableCache<TKey, TValue, THash>::
insert_with_evict(const TKey& key, const TValue& value, bool& evicted, TKey& evict_key, TValue& evict_value) {
  return getShard(key).insert_with_evict(key, value, evicted, evict_key, evict_value);
}

template <class TKey, class TValue, class THash>
void ThreadSafeScalableCache<TKey, TValue, THash>::
remove(Accessor& ac) {
  getShard(ac.m_hashAccessor->first).remove(ac);
}
